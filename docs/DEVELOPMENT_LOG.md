...
- 기연 발생 후 5초 동안 선택이 없으면 자동으로 지나치고 자동 전투를 재개하도록 보완했다.
...
## 2026-08-29 — 경맥 노드 의미 안내
- 경맥 화면에 번호가 개방 순서라는 설명과 분기별 실제 전투 효과를 추가했다.
- 노드 툴팁·접근성 라벨에도 공격력·방어력·치명타 효과 또는 확장 분기 상태를 표시한다.
- 노드 계산과 저장 구조는 변경하지 않았다.

## 2026-08-29 — 경맥 분기 색상 범례
- 경맥 화면에 외공·내공·검맥·도맥·권맥·경공·암기 분기를 색상 범례로 추가했다.
- 번호, 분기명, 효과가 같은 화면에서 대응되어 노드 선택 기준을 즉시 확인할 수 있다.
- `flutter test` 19개 통과, `flutter analyze` 오류 없음(info 103건), 웹 release build 성공.

## 2026-08-29 — 기연 결과 흐름 단순화
- 선택 직후 표시되던 `기연의 결말` 중간 모달을 제거했다.
- 선택 결과는 즉시 메인 화면으로 복귀하고 전투 로그에 기록되며, 자동 전투 흐름이 한 단계로 이어진다.
- 기존 보상 계산·저장·기연 선택 결과는 유지하고 관련 회귀 테스트를 갱신했다.
- `flutter test` 19개 통과, `flutter analyze` 오류 없음(info 103건), 웹 release build 성공.

## 2026-08-29 — 기연 선택 카운트다운
- 기연 선택 화면에 남은 초를 표시하고, 매초 화면을 갱신하도록 보완했다.
- 5초 유예가 끝나면 자동으로 지나쳐 전투가 재개되며, 카운트다운 시작값 회귀 테스트를 추가했다.
- `flutter test` 20개 통과, `flutter analyze` 오류 없음(info 103건), 웹 release build 성공.

## 2026-08-29 — 무도 보유 무공 중심 표시
- 무도 화면에서 미습득 무공을 긴 목록으로 반복 표시하지 않고, 보유 무공만 기본 노출하도록 정리했다.
- 상단에 보유/전체 수와 잠긴 무공 개수를 안내해 해금 진행 정보는 유지한다.
- 무공 세팅·숙련도·전투 발동 로직은 변경하지 않았다.
- `flutter test` 20개 통과, `flutter analyze` 오류 없음(info 104건), 웹 release build 성공.

## 2026-08-29 — 경맥 화면 세로 밀도 개선
- 상단 경맥 설명을 한 줄 중심으로 압축하고, 도표 캔버스 높이와 중심 좌표를 조정했다.
- 320px 세로 화면에서도 하단 노드가 네비게이션 아래로 밀리지 않으며 확대·축소·드래그 이동은 유지된다.
- `flutter test` 20개 통과, `flutter analyze` 오류 없음(info 104건), 웹 release build 성공.

## 2026-08-29 — 경맥 가장자리 노드 선택 보강
- InteractiveViewer 이동 여백을 확대하고 클리핑을 해제해 좌우·하단 노드를 중앙으로 가져올 수 있게 했다.
...
## 2026-08-29 — 경맥 도표 클리핑 보정
- 도표 이동 시 경맥 영역을 벗어나 상단 설명·범례를 덮던 문제를 확인했다.
...
## 2026-08-31 — 경맥 화면 높이 기준 초기 축소 보정
- 세로 화면에서 폭 기준 0.7배만 적용되어 도표 하단이 하단 탭에 가려지는 현상을 확인했다.
- 차트 영역의 폭·높이 중 작은 비율을 사용해 초기 배율을 계산하고, 최소 0.5배까지 전체 트리가 들어오도록 조정했다.
- `flutter test` 20개 통과, `flutter analyze` 오류 없음(info 104건), 웹 release build 성공.

## 2026-09-07 — 전체 품질 감사 및 전투·기연 흐름 보정
- 전체 화면·게임 루프를 점검하고 우선순위별 개선 목록을 `docs/overall_quality_audit.md`에 기록했다.
- 기연 경험치 보상이 즉시 레벨업되도록 공통 경험치 처리 경로를 추가했다.
- 보스 처치 여부를 판정 시작 시점에 고정해 일반 기연·장비 드랍 판정이 보스 보상에 섞이지 않도록 수정했다.
- 회귀 테스트 1건을 추가해 총 21개 테스트가 통과했다.

## 2026-09-07 — 지역별 진행도 저장·표시 보정
- 강호 메인에 고정되어 있던 `68%` 진행도 표시를 지역별 일반 적 처치 누적과 보스 격파 상태 기반으로 교체했다.
- 기존 저장 데이터에는 `areaKills` 기본값을 적용하고, 신규 저장부터 지역별 처치 수를 기록한다.
- 기연 경험치 즉시 레벨업·보스 보상 판정 보정과 함께 회귀 테스트를 추가해 총 22개 테스트가 통과했다.

## 2026-09-07 — 저장 데이터 타입·범위 방어 보강
- 숫자·문자열·목록 타입이 손상된 로컬 저장값을 기본값과 안전한 범위로 보정하도록 파싱 경로를 보강했다.
- 장비 품질·잠금 상태, 무공 목록, 경맥 노드, 장비 목록이 비정상 타입이어도 부팅이 중단되지 않도록 했다.
- `flutter test` 22개 통과, `flutter analyze` 오류 없음(info 105건), 웹 release build 성공.

## 2026-09-07 — 무공별 숙련도 기록·표시
- 활성 무공의 실제 발동 시 개별 숙련도를 1씩 누적하고 로컬 저장 데이터에 기록하도록 했다.
- 보유 무공 목록에 무공별 숙련도를 표시해 전체 숙련도와 실제 사용 숙련도를 구분했다.
- `flutter test` 23개 통과, `flutter analyze` 오류 없음(info 105건), 웹 release build 성공.

## 2026-09-08 — 320×700 화면 overflow 회귀 점검
- 강호·무인·무도·행낭·강호록 핵심 위젯을 320×700 뷰포트에서 순차 렌더링하는 회귀 테스트를 추가했다.
- 테스트에서 `overflowed` 오류가 발생하지 않음을 확인했다.
- `flutter test` 24개 통과, `flutter analyze` 오류 없음(info 105건), 웹 release build 성공.

## 2026-09-08 — 오프라인 보상 중복 수령 방지
- 오프라인 보상 버튼의 빠른 연속 호출로 보상이 중복 계산될 가능성을 차단했다.
- 1회 수령 이후 동일 흐름을 다시 호출해도 처치 수·은자가 추가되지 않는 회귀 테스트를 추가했다.
- `flutter test` 25개 통과, `flutter analyze` 오류 없음(info 105건), 웹 release build 성공.

## 2026-09-08 — 무공 저장·엔딩 스모크 테스트
- 무공별 숙련도가 저장 후 새 Game 인스턴스에 복구되는지 검증했다.
...
## 2026-09-08 — 경맥 분기 효과 전투 반영
- 외공·내공·검맥 외에도 도맥·권맥·경공·암기 분기의 효과가 공격력·방어력·회피·무공 위력에 반영되도록 보정했다.
...
## 2026-09-08 — 행낭·강호록 작은 화면 보완
- 행낭 보관함 요약 패널에 전용 보물 상자 아트를 사용해 화면의 시각적 기준점을 강화했다.
- 강호록 요약 패널에 전용 도장 아트를 사용하고, 5개 기록 탭에 선택 상태·버튼 Semantics를 추가했다.
...
## 2026-09-08 — 무인 빈 장비 슬롯 실루엣 보완
- 미장착 장비 슬롯에도 부위별 전용 아트를 낮은 불투명도로 표시해 무기·머리·의복·손·신발·허리띠·목걸이·옥패 위치를 즉시 구분할 수 있게 했다.
